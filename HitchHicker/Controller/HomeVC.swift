...
class HomeVC: UIViewController, Alertable {
    ...
    override func viewDidLoad() {
        ...
        UpdateService.instance.observeTrips { (tripDict) in
            if let tripDict = tripDict {
                ...
                let acceptanceStatus = tripDict[TRIP_ACCEPTED] as! Bool
    ...
    override func viewWillAppear(_ animated: Bool) {
                    ...
                    if let tripSnapshot = tripSnapshot.children.allObjects as? [DataSnapshot] {
                        for trip in tripSnapshot {
                            if trip.childSnapshot(forPath: DRIVER_KEY).value as? String == currentUserID! {
                                ...
                                self.actionBtn.setTitle(MSG_GET_DIRECTION, for: .normal)
        ...
        DataService.instance.REF_TRIPS.observe(.childRemoved, with: { (removeTripSnapshot) in
            ...
            if removeTripDict?[DRIVER_KEY] != nil {
                DataService.instance.REF_DRIVERS.child(removeTripDict?[DRIVER_KEY] as! String).updateChildValues([DRIVER_IS_ON_TRIP: false])
            }
            ...
            DataService.instance.userIsDriver(userKey: currentUserID!, handler: { (isDriver) in
                if isDriver == true {
                    ...
                    self.actionBtn.animateBtn(shouldLoad: false, withMessage: MSG_REQUEST_RIDE)
        ...
        DataService.instance.REF_DRIVERS.observeSingleEvent(of: .value, with:  { (snapshot) in
            if let driverSnapshot = snapshot.children.allObjects as? [DataSnapshot] {
                for driver in driverSnapshot {
                    if driver.childSnapshot(forPath: USER_IS_DRIVER).value as? Bool == true {
        ...
        DataService.instance.userIsDriver(userKey: currentUserID!) { (status) in
            if status == false {
                DataService.instance.REF_TRIPS.child(currentUserID!).observe(.value, with: { (tripSnapshot) in
                    ...
                    if tripDict?[TRIP_ACCEPTED] as? Bool == true {
                        self.removeOverlaysAndAnnotations(forDriver: false, forPassenger: true)
                            ...
                            if let driverSnapshot = driverSnapshot.children.allObjects as? [DataSnapshot] {
                                for driver in driverSnapshot {
                                    if driver.key == driverID {
                                        ...
                                        self.actionBtn.animateBtn(shouldLoad: false, withMessage: MSG_DRIVER_COMING)
            ...
            DataService.instance.driverIsOnTrip(driverkey: (Auth.auth().currentUser?.uid)!, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
                    DataService.instance.REF_TRIPS.child(tripKey!).observe(.value, with: { (tripSnapshot) in
                        ...
                        pickupMapItem.name = MSG_PASSENGER_PICKUP_POINT
...
extension HomeVC: MKMapViewDelegate {
    ...
    func setCustomRegion(forAnnotationType type: AnnotationType, withCoordinate coordinate: CLLocationCoordinate2D) {
        if type == .pickup {
            ...
            let destinatioRegion = CLCircularRegion(center: coordinate, radius: 100, identifier: REGION_DESTINATION)
